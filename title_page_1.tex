...
Combustion is the development of chemical reactions of oxido-reduction, strongly exothermic and very fast, due to heat and catalyst products accumulation. Catalysis is the process of increasing the rate of a chemical reaction by adding a catalyst, which is not consumed in the reaction. This substance provides an alternative reaction with a lower activation energy. The cause of these reaction are flames:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textit{liquid fuel burners}: the fuel is injected in form of spray. The air jet is introduced with a swirl motion, which causes a pressure drop and recirculation zone close to the central axis of the burner.
\end{itemize}

\subsection{Internal combustion engines}

\begin{itemize}
    \item \textit{Otto}: the combustion process in this spark-ignited engine, develop through a premixed flame with a laminar or turbulent flame propagation.
    \item  \textit{Diesel}: the combustion process develops through a diffusion flame and the flame speed propagation is turbulent.
\end{itemize}

\subsection{Ramjets}

Can be used above $Mach 2$. The gas velocity at the entrance of the combustion chamber is much faster than the flame propagation speed. The recirculation zone grants a permanence of the gas for several milliseconds.
...
Flame is a discontinuity surface separating reactants from combustion products. Its peculiar property is the capability of self-sustaining and propagation. They can be classified as:
...
It's the speed relative to the fresh mixture. It's a constant for a specific mixture and it is proportional to the amount of mixture that in the time unit can be changed into combustion products.
...
It's the temperature of combustion products in an adiabatic system. It depends on the fuel and oxidizer nature and on the mixture ratio.
...
For a generic chemical equilibrium the mathematical formalism proposed by \textbf{Penner} is considered:
...
The standard enthalpy of formation $\Delta h_{f}^{0}$ of a substance is the change in enthalpy during the formation of 1 mole of the substance with all substances in their standard states. The standard enthalpy of formation is a state property, therefore we are only interested in the initial and final state.
...
If the final temperature of the products is the same of the temperature of the reactants, the energy balance does not include contributions linked to the change of temperature but only contributions of chemical nature (enthalpy of formation):
...
\end{itemize}
the thermochemical enthalpy totally involved, is given by:
...
Bond energy is the energy required to break a bond between two atoms. Usually, this energy depends on the distance between atoms. It is necessary to consider the resonance in the molecule (delocalized electrons), which gives higher values of the standard formatioon enthalpy.
...
\begin{equation}
  -\Delta h_{r} = C_{P}\Delta T_{adiabatic}
\end{equation}
\begin{equation}
...
In the picture it is possible to see the trend of the adiabatic flame temperature. Significant increases of $T_{i}$ cause modest increases of $T_{ad}$ due to \textit{dissociation reactions}.
...
\begin{equation}
    \frac{t_{chemical}^{*}}{t_{fluid}^{*}} << 1
\end{equation}

A system is in a stable equilibrium when its state can be changed reversibly by an infinitesimal amount. Such system obeys to the relationship:
...
where the \textbf{Gibbs free energy}, is:
...
The criterium of stable equilibrium to small perturbations leads to:
...
where $\Delta F^{0}(T)$ is the \textbf{variation of standard free energy} (1 atm, substances at standard thermodynamic state) and $K_{p}$ is the \textbf{equilibrium constant} and $p_{j}$ are the partial pressures of the mixture.

\subsection{The physical meaning of $K_{p}$}

The more negative $\Delta F^{0}$ is, the larger is $K_{p}$. This means that the reaction is more spontaneous. When the free energy of the reactants and products is the same, the reaction has no tendency to proceed in either direction. The practical importance of $K_{p}$ is that it is independent on total pressure and can be a unique function of the temperature:
...
$T_{ref}$ is the reference temperature of the available tables.\\
...
where $h_{T}^{0}-h_{T_{ref}}^{0} = \int_{T_{ref}}^{T} Cp_{j}(T)dT$ are tabulated values of the molar thermal enthalpy. The temperature $T_{C}$ resulting from the conservation energy equation is called \textbf{flame temperature}. It increases if enthalpy lost to the environment decreases. If $\Delta h_{ext}=0$, the adiabatic flame temperature is the maximum admissible temperature in the combustion chamber.
...
This is the second part of the uncoupled problem. In this case it is known the working pressure $p_{c}$, $T_{j}$, $T_{c}$. But the mixture composition is unknown:
...
Advantages given by the aluminum powders:
\begin{itemize}
...
aluminum is an interesting metal fuel due to its stability, low toxicity, low cost, availability. Powders available:
\begin{itemize}
  \item nano-aluminum: 50-200 nm
  \item micro-aluminum: 5-50 $\mu$m
  \item activated aluminum: processed to enhance reactivity
\end{itemize}
 Some of the disadvantages of aluminum are the high temperature that causes thermal stresses (heavier protections) the generation of agglomerates which cause a reduction of the real specific impulse. Coarse agglomerates are removed by the use of nanoaluminum. Combustion of fines metals have better quality and may lead to better efficiency. Finer oxidizer grant faster burnig rate.
...
but it also depends on aluminum powder size and on the ingredients mass fraction:
...
\begin{itemize}
  \item safety, no possibility of explosion
  \item start-stop-restart capabilities
  \item low const
  \item higher specific imoulse than solid rocket motors and higher density-specific impulse than liquid bipropellants engines
  \item ability to smoothly change thrust over a wide range
\end{itemize}
The disadvantages are:
\begin{itemize}
  \item mixture ratio, hence, specific impulse will vary during steady-state operations
  \item lower density-specific impulse than solid propellant systems
  \item unproven feasibility at large scale
\end{itemize}

\begin{figure}[!ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/hybridengine.png}
\end{figure}

\subsection{Propellants}

The propellant system of choice for large hybrid booster applications is $\textit{liquid oxygen (LOX)}$ and $\textit{HTPB fuel}$.\\
HTPB stands for  hydroxylterminated polybutadiene, in solid rocket motors it is used as the binder to consolidate the aluminum fuel and ammonium perchlorate oxidizer matrix. In a hybrid, HTPB becomes the entire fuel constituent. HTPB is low cost and will not self-deflagrate under any conditions.

\subsection{Specific impulse}

Theoretical specific imoulse of several oxidizers reacting with h
HTPB as a function of the mixture ratio:

\begin{figure}[!ht]
\centering
\includegraphics[width=0.6\textwidth]{figures/htpb.png}
\end{figure}

\subsection{Aluminum addition}

Where a smoky exhaust is not a detriment, hybrid propellants may benefit from addition of powdered aluminum to the fuel. This increase the combustion temperature, reduces the stoichiometric mixture ratio (more oxidizer is needed) and increase fuel density as well as overall density-specific impulse but reduces the specific impulse. This occurs because the increase in the flame temperature gained by adding aluminum does not compensate for the increase in molecular weight of the exhaust products.\\
Metal particles addition can change the combustion processes, which can be expressed by the Damkholer number:

\begin{equation}
  Da = \frac{flow time scale}{chemical time scale}
\end{equation}

\begin{figure}[!ht]
\centering
\includegraphics[width=0.8\textwidth]{figures/damkholer.png}
\end{figure}

\subsection{Effect of n-Al particles (nano-sized aluminum)}

In the figure it is possible to see the effect of n-Al particles on regression an mass-burning rate:

\begin{figure}[!ht]
\centering
\includegraphics[width=0.4\textwidth]{figures/nal.png}
\end{figure}

In particular, the evaluation of UF-Al (ultrafine aluminum) was conducte by adding a specific amount of particles(4, 12 or 20\%) to one of the solid fuels, while the other was pure HTPB to allow direct measurements. At a particle addition of 20\% of UF-Al an icrease in the regression rate was seen compared to those of the pure HTPB fuels.\\
Ignition and combustion of the UF-Al particles provides more heat to the solid surface and thus generate a higher regression rate than the nonmetallized case. Micrometer sized aluminum particle addition demonstarted increase in the regression rate but not the same degree as nanosized particle. This increase in regression rate for solid fuel with micrometer-sized aluminum particles was attributed to the increase in heat transfer to the surface as well as an increase in flame temperature.

\subsection{Combustion process}

In the figure it is possible to see the diffusion-controlled combustion processed in a hybrid rocket engine as well as the boundary layer:

\begin{figure}[!ht]
\centering
\includegraphics[width=0.6\textwidth]{figures/hybrid2.png}
\end{figure}

\subsection{Regression rate}

The mechanisms of heat transfer to the fuel grain surface in a hybrid are convection and radiation. In a non-metallized fuel, heat transferred by convection is much larger than that transferred by radiation. As a result, the fuel grain regression can be explored via a convective heat transfer analysis in a turbolent layer. Considering an energy balance at the fuel grain surface, it is possible to derive an expression for the fuel surface regression rate:

\begin{equation}
  \dot{r}=0.036 \frac{G^{0.8}}{\rho_{f}}\bigg(\frac{\mu}{x} \bigg)^{0.2}\beta^{0.23}
\end{equation}

where G is the free stream propellant mass velocity at any given location x, $\mu$ is the gas viscosity and $\beta$ is the non-dimensionalized fuel mass flux (blowing coefficient). The equation indicates that hybrid fuel regression rate is strongly dependent on G and weakly dependent on axial location x a nd fuel blowing characteristic $\beta$. Also, the regression rate is not explicity dependent on pressure.\\
A further simplification can be made for purposes of preliminary engineering design:

\begin{equation}
  r_{b} = a (G_{O}x)^{n}
\end{equation}

where $G_{O}$ is the oxidizer mass velocity.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.8\textwidth]{figures/hybrid4.png}
\end{figure}
